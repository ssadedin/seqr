...
class VariantAnnotator():
    ...
    def add_preannotated_vcf_file(self, vcf_file_path, force=False):
        ...
        if ann_field is None:
            raise ValueError("ERROR: CSQ/ANN field not found in %s. Was this VCF annotated with VEP?" % vcf_file_path)
        ...
        actual_csq_fields_string = str(r.infos[ann_field].desc).split("Format:")[1].strip()
        actual_csq_fields = set(actual_csq_fields_string.split("|"))
        if len(expected_csq_fields - actual_csq_fields) > 0:
            print "WARNING: VEP did not add all expected CSQ fields to the VCF. The VCF's CSQ = %s and is missing these fields: %s" % (actual_csq_fields_string, ",".join(expected_csq_fields - actual_csq_fields))
